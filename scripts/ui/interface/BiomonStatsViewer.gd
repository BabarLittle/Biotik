...
func load_biomon_stats(values: Dictionary = {}, base_stats=false):
	...
	for key in values.keys():
		dictionnary_line[key].label.text = str(values[key])
		new_polygon_values.append(get_point_position(values[key], dictionnary_line[key], max_value))
